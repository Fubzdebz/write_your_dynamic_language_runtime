    ...
    private static void visit(Expr expression, JSObject env, MethodVisitor mv, FunDictionary dictionary) {
        switch(expression) {
            case Block(List<Expr> exprs, int lineNumber) -> {
                ...
                // for each expression
                for(var expr : exprs) {
                    //CRÉATION d'un label pour chaque expression
                    var label = new Label();
            ...
            //Maintenant, on use ast pour javas
            ...
            case Literal(Integer integer, int lineNumber) -> {
                ...
                //**non supporté nativement par la JVM, on utilise une ConstantDynamic pour que le BSM puisse faire le travail de boxage
            }
            case Literal(String s, int lineNumber) -> {
                ...
                mv.visitLdcInsn(s);//Supporter par la MV
            }
            ...
            case Call(Expr qualifier, List<Expr> args, int lineNumber) -> {
                ...
                // generate an invokedynamic
                var desc = "(" + "Ljava/lang/Object;".repeat(2 + args.size()) + ")Ljava/lang/Object;";//MethodType.genericMethodType(1 + args.size()).toMethodDescriptorString();
                mv.visitInvokeDynamicInsn("funcall", /*descripteur*/ desc, BSM_FUNCALL);
            }
            ...
            case Identifier(String name, int lineNumber) -> {
                ...
                var slot = env.lookupOrDefault(name, null);
                //chercher dans l'env une certaine chaine de caractère, si elle n'existe pas, on retourne null
                // if it does not exist
                if(slot == null) {
                    // generate an invokedynamic to do a lookup with BSM_LOOKUP
                    mv.visitInvokeDynamicInsn("lookup", "()Ljava/lang/Object;", BSM_LOOKUP, name);
                } else {
                    // otherwise, load the local variable at the slot
                    mv.visitVarInsn(Opcodes.ALOAD, (int) slot);
                }
